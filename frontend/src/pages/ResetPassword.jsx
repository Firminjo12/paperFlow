...
const ResetPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Mock success for transition
            // In a real app, we would verify a reset token from the URL here
            console.log("Mock: Simulation de maj mot de passe effectuée.");
            setSuccess(true);
            setTimeout(() => {
                navigate('/login');
            }, 3000);
            ...
            >
                ...
                <div className="bg-white dark:bg-[#0d1120] border border-slate-200 dark:border-blue-500/20 rounded-[40px] p-8 md:p-10 shadow-2xl relative transition-colors">
                    <div className="space-y-2 mb-8">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-sm">Définissez votre nouveau mot de passe sécurisé.</p>
                    </div>

                    {error && (
                        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl flex items-center gap-3 mb-6 text-sm font-medium transition-all">
                            <AlertCircle size={20} />
                            {error}
                        </div>
                    )}
                    ...
                    {success ? (
                        <div className="bg-green-500/10 border border-green-500/20 text-green-400 p-8 rounded-[32px] flex flex-col items-center text-center gap-4 transition-all">
                            <div className="w-16 h-16 bg-green-500/10 rounded-full flex items-center justify-center">
                                <CheckCircle size={32} />
                            </div>
                            <h3 className="text-xl font-bold">Réinitialisation réussie !</h3>
                            <p className="text-slate-400 text-sm leading-relaxed">Votre mot de passe a été mis à jour. Vous allez être redirigé vers la page de connexion.</p>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-2">Nouveau mot de passe</label>
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        required
                                        placeholder="••••••••"
                                        className="w-full h-14 bg-slate-50 dark:bg-[#0a0f1e] border border-slate-200 dark:border-white/5 rounded-2xl pl-12 pr-12 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                                    />
                                    <button
                                        type="button"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-2">Confirmer le mot de passe</label>
                                <div className="relative group">
                                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 group-focus-within:text-blue-500 transition-colors" size={20} />
                                    <input
                                        type={showPassword ? "text" : "password"}
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full h-14 bg-gradient-to-r from-[#388bfd] to-[#5e3aee] text-white font-black text-sm uppercase tracking-widest rounded-2xl shadow-lg hover:shadow-blue-500/20 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                            >
                                {loading ? "Mise à jour..." : <><Save size={20} /> Enregistrer mon mot de passe</>}
                            </button>
                        </form>
                    )}
                </div>
